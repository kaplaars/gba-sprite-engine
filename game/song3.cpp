...
song3::song3(const std::shared_ptr<GBAEngine> &engine) : Scene(engine) {}

std::vector<Sprite *> song3::sprites() {
...
std::vector<Background *> song3::backgrounds() {
    return {
            bg.get()
    };
}

void song3::tick(u16 keys) {
//ga terug naar startscene
    ...
    TextStream::instance().setText(std::to_string(score3), 2, 1);
...
void song3::load() {
    ...
    engine->enqueueMusic(music1, music1_bytes, (512*1000));
